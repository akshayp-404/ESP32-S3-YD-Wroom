...
/// @brief When using the ESP32 as a Keyboard Manager the LEDs will be connected to GPIO pins on the ESP32 board.
///        They are managed here and use the static variables numLock, scrollLock and capsLock.
void updateLeds()
{
  digitalWrite(NUM_LOCK_LED, bNumLock ? HIGH : LOW); //Num Lock
  digitalWrite(CAPS_LOCK_LED, bCapsLock ? HIGH : LOW); //Caps Lock
  //digitalWrite(SCROLL_LOCK_LED, bScrollLock ? HIGH : LOW); //Scroll Lock
}
